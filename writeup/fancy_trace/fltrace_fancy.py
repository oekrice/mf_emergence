...
class trace_fieldlines():
    def __init__(self, run, snap, show = 0):
    ...
    def plot_emiss(self):
        ...
        xsplot = np.linspace(-130,130,self.zsum.shape[0]+1)
        ysplot = np.linspace(-130,130,self.zsum.shape[1]+1)
        zsplot = np.linspace(0,130,self.xsum.shape[1]+1)
        ...
        for i in range(3):
            ...
            #vmax = 1.0
            print(i, vmax)
            ax.pcolormesh(ranges[i][0], ranges[i][1],toplot.T, vmin = 0, vmax = vmax, cmap = 'inferno')
            ax.set_aspect('equal')
    ...
    def set_starts(self):
        ...
        for i in range(nx_lines):
             for j in range(ny_lines):
                 xstart = 0.9*self.xs[0] + (i/nx_lines)*0.9*(self.xs[-1] - self.xs[0])
                 ystart = 0.9*self.ys[0] + (j/ny_lines)*0.9*(self.ys[-1] - self.ys[0])
...
for run in range(run, run+1):
    for snap in range(snap_min, snap_max, skip):
    	...
    	trace_fieldlines(run = run, snap = snap, show = show)
